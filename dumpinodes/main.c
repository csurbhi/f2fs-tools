		...
		int *total_files)
{
	...
	for (idx = 0; idx < ADDRS_PER_BLOCK; idx++) {
		...
		if (LAYOUT_SCORE == action) {
			printf("\n nid:%d %6dblock: %8lu segno:%6lu ", node->footer.ino, *total_blks, blkaddr, GET_SEGNO(sbi, blkaddr));
...
float process_inode_info(struct f2fs_sb_info *sbi, nid_t nid, struct f2fs_inode *inode)
{
	...
	if (prevnr != 0) {
		printf("\n nid:%d %6dblock: %8lu segno:%6lu", nid, total_blks, prevnr, GET_SEGNO(sbi, prevnr));
		total_blks++;
	}
	...
	for (i = 1; i < ADDRS_PER_INODE(inode); i++) {
		...
		if (blknr == 0)
			continue;
		printf("\n nid:%d %6dblock: %8lu segno:%6lu ", nid, total_blks, blknr, GET_SEGNO(sbi, blknr));
